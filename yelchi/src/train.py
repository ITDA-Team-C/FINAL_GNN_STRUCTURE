"""Training loop for YelpChi fraud detection."""
import os
import json
import random
import argparse
...
from yelchi.src.data_loader import load_yelchi, summary, RELATION_NAMES
from yelchi.src.metrics import calculate_metrics, find_best_threshold, print_metrics
from yelchi.src.models import (
...
def run_single(model_name, mat_path, device, out_dir="yelchi/outputs",
               epochs=200, hidden_dim=128, num_layers=3, dropout=0.3, lr=1e-3,
               patience=20, val_interval=5, seed=42):
    set_seed(seed)
    print(f"\n{'='*70}\n[YelpChi] training {model_name}\n{'='*70}")
    x, y, edge_index_dict, train_mask, valid_mask, test_mask = load_yelchi(mat_path, seed=seed)
