"""Run 7 YelpChi models with 5 random seeds (35 runs total).

Each (model, seed) pair saves to `yelchi/outputs/metrics_<model>_seed{N}.json`.
After completion, aggregates mean ± std per model across seeds.

Usage:
    python 5x_run_all_yelchi.py
    python 5x_run_all_yelchi.py --seeds 42 123
    python 5x_run_all_yelchi.py --only cage_carerf
    python 5x_run_all_yelchi.py --mat-path yelchi/data/YelpChi.mat
    python 5x_run_all_yelchi.py --epochs 100
    python 5x_run_all_yelchi.py --continue-on-error
    python 5x_run_all_yelchi.py --dry-run
"""
import argparse
import glob
import json
...
def aggregate(out_dir="yelchi/outputs"):
    ...
    print("\n" + "=" * 110)
    print(f"=== YelpChi Multi-seed Aggregation (n={len(groups)} models) ===")
    print("=" * 110)
...
def main():
    ...
    print(f"=== YelpChi: {len(plan)} models × {len(seeds)} seeds = {total_runs} runs on {device} ===")
